...
class MealIntakeBloc extends Bloc<MealIntakeEvent, MealIntakeState> {
  ...
  Future<void> _onLoadMealIntakes(
      LoadMealIntakes event, Emitter<MealIntakeState> emit) async {
    mealIntakeRepository.getMealIntakes(event.userId).listen((mealIntakes) {
      add(MealIntakesUpdated(mealIntakes));
    });
  }

  Future<void> _onAddMealIntake(
      AddMealIntake event, Emitter<MealIntakeState> emit) async {
    await mealIntakeRepository.addMealIntake(event.mealIntake);
  }

  Future<void> _onUpdateMealIntake(
      UpdateMealIntake event, Emitter<MealIntakeState> emit) async {
    await mealIntakeRepository.updateMealIntake(
        event.mealIntake, event.mealIntakeId);
  }

  Future<void> _onDeleteMealIntake(
      DeleteMealIntake event, Emitter<MealIntakeState> emit) async {
    await mealIntakeRepository.deleteMealIntake(event.mealIntakeId);
  }

  void _onMealIntakesUpdated(
      MealIntakesUpdated event, Emitter<MealIntakeState> emit) {
